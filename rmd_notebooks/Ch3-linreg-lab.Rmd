...
- *Question* (what makes using `coef()` safer?)
...
- the 95\,\% confidence interval associated with a `lstat` value of 10 is $(24.47, 25.63)$,  (i.e., you are 95% confident that the mean value of the predicted home value with the given input parameters (lstat = 10) is within 24.47 and 25.63) and,
- the 95\,\% prediction interval is $(12.828, 37.28)$, (i.e., you are 95% confident that the next home that will be sold with these input parameters (`lstat=10`) is within 12.82 amd 37.27).
- Since the predicted value of a single sample is more uncertain that the mean, the prediction interval is always larger than the confidence interval. For more details see [SO-Link1](https://stats.stackexchange.com/a/271232/85847) and [StatsByJim](https://statisticsbyjim.com/hypothesis-testing/confidence-prediction-tolerance-intervals/)
...
- The feature `age` which was significant when used with `lstat` is not so significant in the presence of other variables. This suggest interaction between the features.
...
- VIF of the $j^{th}$ predictor is defined as
  \[ VIF_j = \frac{1}{1-R^2_j}\]
where $R^2_j$ is obtained by regressing the $j^{th}$ predictor on the remaining predictors.
...
- More details on VIF are available on [Stat 462](https://online.stat.psu.edu/stat462/node/180/)

Most VIF's are low to moderate for this data. The `car` package is not part of the base `R` installation so it must be downloaded the first time you use it via the `install.packages()` function in `R`.
...
- The residual plot appears like the errors are increasing for higher factor values (heteroscadasticity)
...
- We can also create interaction variables between quantitative (price) and qualitative(ShelveLoc) variables


```{r}
lm.fit <- lm(Sales ~ . + Income:Advertising + Price:ShelveLoc, 
    data = Carseats)
summary(lm.fit)
contrasts(ShelveLoc)
```
The interaction between Price and ShelveLoc has minimal influence on the target variable.

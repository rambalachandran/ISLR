...
```
 ...
 - Hitting the *up* arrow multiple times will display the previous commands, which can then be edited. This is useful since one often wishes to repeat a similar command. 
  - `Ctrl+R` can be used in VSCode Terminal to search historical commands
 - In addition, typing `?funcname` will always cause `R` to open a new help file window with additional information about the function `funcname()`.
 ...
 - The length of the larger vector must be a  multiple of shorter object length
...
A[-c(1, 3), ]
A[-c(1, 3), -c(1, 3, 4)]
# retains matrix type due to drop argument being false
...
- The `pairs()` cannot handle non-numerical columns (ex: `as.factor()` output)
...
## Questions
1. Is there a difference between integer and float. `class(5) and class(pi)` both return output as numeric